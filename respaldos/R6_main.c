//se realiza 1 test de escritura al principio y luego solo se leen repetidamente las entradas
...
#define I2C_ADDRESS_INPUT ESP_IO_EXPANDER_I2C_HT8574_ADDRESS_100  /*!< Dirección I2C del expansor de entrada */
    ...
    while (true)
    {
        ...
        if (ret == ESP_OK)
        {
            ...
            for (int pin = 0; pin < 6; pin++)
            {
                ...
                ESP_LOGI(TAG, "Entrada %d: %d (%s)", pin + 1, pin_state, state_text);
            }
        }
        else
        {
...
void io_control_task(void *arg)
{
    ...
    {
        ESP_LOGI(TAG, "Ciclo %d de %d", cycle + 1, 1);
    ...
    // Crear la tarea de monitoreo de entradas después de que el ciclo de control de salidas ha terminado
