//se cambia el LOG
//1 cicly de test de escritura de la salida 1 as la 6
//luego se realiza repetidamente la lectura de las entradas y tambien del estado de las salidas
/*
I (117526) ht8574_control: ===== Estado de Entradas =====
I (117526) ht8574_control: Entrada 1: 0 (ON)
I (117526) ht8574_control: Entrada 2: 0 (ON)
I (117526) ht8574_control: Entrada 3: 0 (ON)
I (117536) ht8574_control: Entrada 4: 0 (ON)
I (117536) ht8574_control: Entrada 5: 0 (ON)
I (117536) ht8574_control: Entrada 6: 0 (ON)
I (117546) ht8574_control: ===== Estado de Salidas =====
I (117556) ht8574_control: Salida 1: 1 (OFF)
I (117556) ht8574_control: Salida 2: 1 (OFF)
I (117566) ht8574_control: Salida 3: 1 (OFF)
I (117566) ht8574_control: Salida 4: 1 (OFF)
I (117576) ht8574_control: Salida 5: 1 (OFF)
I (117576) ht8574_control: Salida 6: 1 (OFF)
*/
...
void io_input_monitor_task(void *arg)
{
    ...
    while (true)
    {
        ...
        if (ret == ESP_OK)
        {
            ESP_LOGI(TAG_HT8574, "===== Estado de Entradas =====");
            for (int pin = 0; pin < 6; pin++)
            {
                int pin_state = (input_state >> pin) & 0x1; // Obtener el estado del pin
                const char *state_text = (pin_state == 0) ? "ON" : "OFF";
                ESP_LOGI(TAG_HT8574, "Entrada %d: %d (%s)", pin + 1, pin_state, state_text);
            }
        }
        else
        {
        ...
        if (ret == ESP_OK)
        {
            ESP_LOGI(TAG_HT8574, "===== Estado de Salidas =====");
            for (int pin = 0; pin < 6; pin++)
            {
                int pin_state = (output_state >> pin) & 0x1; // Obtener el estado del pin
                const char *state_text = (pin_state == 0) ? "ON" : "OFF";
                ESP_LOGI(TAG_HT8574, "Salida %d: %d (%s)", pin + 1, pin_state, state_text);
            }
        }
        else
        {
