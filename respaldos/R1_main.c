//prende y apaga todas las salidas al mismo tiempo cada un segundo.
...
#define OUTPUT_PINS_MASK    (IO_EXPANDER_PIN_NUM_1 | IO_EXPANDER_PIN_NUM_2 | IO_EXPANDER_PIN_NUM_3 | \
                             IO_EXPANDER_PIN_NUM_4 | IO_EXPANDER_PIN_NUM_5 | IO_EXPANDER_PIN_NUM_6)
...
// Tarea principal para encender y apagar las salidas 1 a 6 cada segundo
void io_control_task(void *arg)
{
    while (true) {
        // Establece el nivel bajo en las salidas (apaga las salidas)
        ESP_LOGI(TAG, "Apagando salidas 1 a 6");
        esp_io_expander_set_level(io_expander, OUTPUT_PINS_MASK, 0);
        vTaskDelay(pdMS_TO_TICKS(1000));  // Espera 1 segundo

        // Establece el nivel alto en las salidas (enciende las salidas)
        ESP_LOGI(TAG, "Encendiendo salidas 1 a 6");
        esp_io_expander_set_level(io_expander, OUTPUT_PINS_MASK, 1);
        vTaskDelay(pdMS_TO_TICKS(1000));  // Espera 1 segundo
    }
}
